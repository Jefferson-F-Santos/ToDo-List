...
function veryItemOfDelete(idTask){
    lisTask.forEach(e => {
        if(idTask === e.id){
            return lisTask.indexOf(e)
        }
    })
}

const deleteItem = () => {
    const posArray = veryItemOfDelete()
    return lisTask.splice(posArray)
}
    ...
    lisTask.forEach(item => {
        containerMain.innerHTML += `
            <div class="content-task">
                ...
                <button class="deleteBtn" onClick="${deleteItem(item.id)}" ><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px"
                    width="24" height="24"
                    viewBox="0 0 24 24"
...
const addTask = () => {
    ...
    const valueInput = shotValueInput()
